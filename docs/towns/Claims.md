...
领地系统让你圈占地图上的土地，保护你的建筑与资源——别人无法破坏方块、打开箱子、偷走物品。系统由服务器自研插件 **PepperClaim** 提供。

## 领地类型

| 类型 | 指令前缀 | 数量 | 说明 |
|---|---|---|---|
| **家园领地** | `/pclaim` | 每人 **1 块** | 你的唯一居所，个人专属，永不因联盟变动受影响 |
| **联盟领地** | `/gclaim` | 按槽位（初始 3 块，至多 6 块） | 属于[联盟](index.md)的公共领地，大家一起建设 |
| **子领地** | `/gclaim sub` | 每块联盟领地最多 10 个 | 联盟领地内的功能分区（如商店区、农田区） |
...
> **引号规则**：大部分命令直接输中文领地名即可（`tp` / `select` / `create` / `resize` / `delete` / `info` 等）；但 `rename`、`set preset`、`flag`、`perm`、`group` 这几个命令的领地名参数**需要加引号**：
> ```
...
- **传送冷却**：领地传送有 30 秒冷却。
- **防重叠与间距**：圈地自动校验重叠、出生点与联盟间距，冲突时红色粒子提示。
...
联盟领地是大家一起建的生产设施（刷怪塔、农场、市场），权限按联盟职位分享，详见[职位与权限](Roles.md)。
...
**Q：中文领地名报错？**
`rename`、`set preset`、`flag`、`perm`、`group` 命令的领地名参数需要加引号：`/pclaim rename "我的家" 新名字`。
